...
class RemindMe:
    ...
    async def remindme(self, ctx,  quantity : int, time_unit : str, *, text : str):
        ...
        Accepts: seconds, minutes, hours, days, weeks, month
        Example:
        ...
        if not time_unit in self.units:
            await self.bot.say("Invalid time unit. Choose seconds/minutes/hours/days/weeks/month")
            return
        if quantity < 1:
        ...
        self.reminders.append({"ID" : author.id, "FUTURE" : future, "TEXT" : text, "CHANNEL" : channel.id})
        ...
        while self is self.bot.get_cog("RemindMe"):
            ...
            for reminder in self.reminders:
                if reminder["FUTURE"] <= int(time.time()):
                    try:
                        await self.bot.send_message(discord.Object(id=reminder["CHANNEL"]), "{}, you asked me to remind you: {}".format(discord.User(id=reminder["ID"]).mention, reminder["TEXT"]))
